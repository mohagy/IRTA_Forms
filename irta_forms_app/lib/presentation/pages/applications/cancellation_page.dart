  ...
  Widget _buildContent(BuildContext context) {
    return Consumer<ApplicationProvider>(
      builder: (context, appProvider, _) {
        // Filter applications to only show Cancellation type
        final allApplications = appProvider.applications;
        final cancellationApplications = allApplications.where((app) => 
          app.formType.toLowerCase().contains('cancellation')
        ).toList();

        // Calculate stats dynamically
        final total = cancellationApplications.length;
        final pending = cancellationApplications.where((app) => 
          app.status == AppConstants.statusSubmitted || 
          app.status == AppConstants.statusReceptionReview
        ).length;
        final inProcess = cancellationApplications.where((app) => 
          app.status == AppConstants.statusVerification ||
          app.status == AppConstants.statusIssuingDecision
        ).length;
        final completed = cancellationApplications.where((app) => 
        ...
        return Column(
          children: [
            // Header
            AppHeader(
              title: 'Cancellation / Withdrawal Requests',
              actions: [
                SizedBox(
                  width: 300,
                  child: TextField(
                ...
                ElevatedButton.icon(
                  ...
                  label: const Text('New Cancellation Request'),
                ),
              ],
            ),
