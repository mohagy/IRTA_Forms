  ...
  Future<void> _saveUser(BuildContext context, UserProvider userProvider, bool isEdit) async {
    ...
    try {
      if (isEdit && widget.user != null) {
        ...
        // Create new user - Note: This only creates the Firestore document
        // The user still needs to be created in Firebase Auth separately
        ...
        if (context.mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                success
                    ? 'User document created. Note: User must also be created in Firebase Auth.'
